...
class MyCachedDataset(Dataset):
    ...
    def __getitem__(self, idx):
        data = np.load(f"{self.path}/{idx}.npz")
        return data['embedding'], data['captions']
...
def collate_fn(
    ...
    # encoder_hidden_states = torch.from_numpy(batch[0][0])
    
    # pad encoder states to max length
    # max_length = 541
    # encoder_hidden_states = []
    # for x, _ in batch:
    #     x = np.concatenate([x, np.zeros((1, max_length - x.shape[1], 768))], axis=1)
    #     x = x.squeeze(axis=0)
...
def get_dataloader(dataset: MyCachedDataset, batch_size: int = 64, shuffle: bool = False):
    return DataLoader(
        dataset=dataset,
...
if __name__ == "__main__":
    ...
    if not args.eval_only:
        ...
        # val_dataset = CachedDataset.load("../cached_data/val_data.pkl")
        # val_dataset = MyCachedDataset("../cached_data/val_data")
        print("Loading cached train dataset...")
        # train_dataset = CachedDataset.load("../cached_data/train_data.pkl")
        # train_dataset = MyCachedDataset("../cached_data/train_extended_data")
        train_dataset = MyCachedDataset("../cached_data/train_blip_extended_data")
        print("Done loading cached datasets.")
