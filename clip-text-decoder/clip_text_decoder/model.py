...
class Decoder(LightningModule):
    ...
    def forward(
        ...
        if self.vision_backbone == "owl:v2":
...
class DecoderInferenceModel:
    ...
    @torch.no_grad()
    def __call__(self, x: Tensor, max_len: int = 64, beam_size: int = 1, input_text=None) -> str:
        ...
        have been generated.
        """
        encoder_hidden_states = x.to(self.device)
        ...
        if input_text is None:
            ...
            input_ids = [self.tokenizer.encode(input_text, return_tensors="pt")[0].to(self.device)]
